...
| --- | --- |
| macOS Apple Silicon | `mail-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `mail-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/mail-gateway.rb
```
...
brew install user/tap/mail-gateway
mail-gateway --version
brew test user/tap/mail-gateway
```
